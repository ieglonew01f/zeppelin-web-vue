  ...
  <div>
    ...
    <div v-show="!hideEditor" class="editor-wrapper">
      ...
      <div v-on:keyup.shift.enter="runParagraph()" class="editor-main">
        <div class="dropdown">
          <span data-toggle="dropdown" class="options"><i data-feather="more-horizontal"></i></span>
...
export default {
  ...
  computed: {
    setOptions: function () {
      const p = this.$store.getters.getParagraphById(this.$props.paragraph.id)
    ...
    getParagraphInit: function () {
      const {id} = this.$props.paragraph
      const isPending = this.$store.getters.isParagraphCreating(id)
